...
pub async fn query<T: AsRef<str>>(sql: T) -> Result<DataSet> {
    ...
    filtered = order_by
        .into_iter()
        .fold(filtered, |acc, (col, desc)| acc.sort(&col, SortOptions { descending: desc, nulls_last: false }));

    if offset.is_some() || limit.is_some() {
        filtered = filtered.slice(offset.unwrap_or(0), limit.unwrap_or(usize::MAX).try_into().unwrap());
    }
